...
def create_comment(
    ...
    current_user: UserModel = Depends(get_current_user)  # 🔑 only authenticated
):
    tea = db.query(TeaModel).filter(TeaModel.id == tea_id).first()
    if not tea:
        raise HTTPException(status_code=404, detail="Tea not found")
...
def update_comment(comment_id: int, comment: CommentUpdateSchema, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)
):
